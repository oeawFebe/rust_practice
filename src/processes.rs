...
fn process_pending(item: Pending, command: String, state: &Map<String, Value>){
    ...
    //A String is allocated in the heap, so the lifetime of the &str contained inside (that is returned by the as_str() method provided by the standard library) is the one of the container,
